...
public class ScrapPostRepositoryImpl implements ScrapPostCustomRepository{
    ...
    public Page<GetUserScrapPost> findUserScrapPost(Long scrapId, Pageable pageable) {
        ...
        QueryResults<GetUserScrapPost> getUserScrapPostQueryResults = queryFactory
                .select(
                        new QScrapPostDTO_GetUserScrapPost(post.id, post.title, post.thumbnailImgPath, trade.tradeStatus, category.name, scrapPost.scrap.count())
                ).from(scrapPost)
